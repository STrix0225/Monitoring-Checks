...
import '../auth/login_screen.dart';
    ...
    class _DashboardNgPageState extends State<DashboardNgPage> {
      ...
      final List<PicLineModel> _picList = [];
          ...
          if (result != null && result.files.isNotEmpty) {
            ...
            try {
              ...
              if (uploadResponse is Map) {
                fileName = uploadResponse['file_name'] ?? uploadResponse['filename'] ?? '';
                print('🔍 Parsed from Map: $fileName');
              } else if (uploadResponse is List) {
                if (uploadResponse.isNotEmpty && uploadResponse[0] is Map) {
                  fileName = uploadResponse[0]['file_name'] ?? uploadResponse[0]['filename'] ?? '';
                  print('🔍 Parsed from List[0]: $fileName');
                }
                ...
                try {
                  ...
                  if (parsed is Map) {
                    ...
                    print('🔍 Parsed from String (Map): $fileName');
                  } else if (parsed is List && parsed.isNotEmpty && parsed[0] is Map) {
                    fileName = parsed[0]['file_name'] ?? parsed[0]['filename'] ?? '';
                    print('🔍 Parsed from String (List): $fileName');
                  }
                } catch (e) {
              ...
              _imagePath = '$fileUrl/file_name/$fileName';
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text("${pic.id_pic} • ${pic.line}", style: const TextStyle(color: Colors.grey, fontSize: 12)),
